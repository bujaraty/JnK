...
var g_baitQuantity = -1;
var g_currentLocation;
var g_checkTimeDelay = 0;
var g_checkTime;
var g_lastKingRewardSumTime;
...
// K_Todo_013
// I need to refactor code in this part (under this whole "if") as small functions later
...
// K_Todo_016
// Why does it use "indexOf" instead of using "getPageVariable"
                // get next horn time
...
//                        lastDateRecorded = new Date();
                ...
                if (hasPuzzleStartIndex >= 0) {
                    ...
                    g_isKingReward = (hasPuzzleString != 'false');

                    if (debug) console.log("Fetched isKingReward: " + g_isKingReward);
                ...
                if (baitQuantityStartIndex >= 0) {
                    ...
                    g_baitQuantity = parseInt(baitQuantityString);

                    if (debug) console.log("Fetched baitQuantity: " + g_baitQuantity);
...
// K_Todo_017
// Why doesn't it use "getPageVariable("user.environment_name")" here?
                ...
                if (locationStartIndex >= 0) {
                    ...
                    g_currentLocation = locationString;

                    if (debug) console.log("Fetched currentLocation: " + g_currentLocation);
...
// K_Todo_018
// Why does it have to look for the last locaiton?
// Probably I have to figure out what the huntLocation is for.
            // get last location
            var huntLocationCookie = getStorage("huntLocation");
            if (isNullOrUndefined(huntLocationCookie)) {
                g_huntLocation = g_currentLocation;
                setStorage("huntLocation", g_currentLocation);
            } else {
                g_huntLocation = huntLocationCookie;
                setStorage("huntLocation", g_huntLocation);
            }
            ...
            if (isNullOrUndefined(lastKingRewardDate)) {
                g_lastKingRewardSumTime = -1;
            } else {
                var lastDate = new Date(lastKingRewardDate);
                g_lastKingRewardSumTime = parseInt((new Date() - lastDate) / 1000);
                lastDate = undefined;
            }
...
//            alert(huntLocation);
// K_Todo_008
// I probably will remove this line later as there is no use
//            // update location
//            displayLocation(huntLocation);
//
...
*/
        ...
        var intervalTime = timeElapsed(g_lastDateRecorded, dateNow);
//        lastDateRecorded = undefined;
        g_lastDateRecorded = dateNow;
        dateNow = undefined;

// K_Todo_020
// I may have to revamp this code as "hornTime -= intervalTime" is not a proper way to do
// I have to make sure that the horn time is appropriately calculated
// One problem might be that when I browse to some other page (esp tournament) the horntime is stop decreasing
        // Update time
        g_hornTime -= intervalTime;
        if (g_lastKingRewardSumTime != -1) {
            g_lastKingRewardSumTime += intervalTime;
        }
        if (enableTrapCheck) {
            g_checkTime -= intervalTime;
        }
...
//            hornTime = 0;
//            soundHorn();
        } else if (enableTrapCheck && g_checkTime <= 0) {
//            // trap check!
//            checkTime = 0;
//            trapCheck();
        } else {
...
/*
// remove later as I'm not going to use this feature
// K_Todo_019
...
function CalculateNextTrapCheckInMinute() {
    if (enableTrapCheck) {
        ...
        g_checkTimeDelay = checkTimeDelayMin + Math.round(Math.random() * (checkTimeDelayMax - checkTimeDelayMin));
        g_checkTime = (now.getMinutes() >= trapCheckTimeDiff) ? 3600 + temp : temp;
        g_checkTime += g_checkTimeDelay;

        if (debug) console.log("CalcNextTrapCheck: " + g_checkTime);
        now = undefined;
        temp = undefined;
    }
}
